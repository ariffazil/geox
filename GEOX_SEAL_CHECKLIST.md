...
1. `geox_physics_compute_ac_risk` → `AC_Risk < 0.15`
...
- [ ] `geox_map_verify_coordinates` — CRS valid, bounds physically possible
...
**Mandatory metabolizers:**
- [ ] `geox_earth3d_verify_structural_integrity` — no overlapping faults, no negative volumes
...
**Mandatory metabolizers:**
- [ ] `geox_well_verify_petrophysics` — no anomalous physics violations (F9)
...
**Mandatory metabolizers:**
- [ ] `geox_time4d_verify_timing` — trap formation ≥ charge timing
...
- [ ] `geox_physics_compute_ac_risk` — `U_phys` penalty applied if unvalidated heat-flow model used
...
- [ ] `geox_physics_judge_verdict` — 888_JUDGE on causal scene passed without HOLD overrides
- [ ] `geox_physics_audit_hold_breach` — no prior HOLD breaches in this prospect lineage
...
- [ ] `geox_physics_fetch_authoritative_state` — ground-truth state vector consulted (or `null` documented with `U_phys` penalty)
...
- [ ] `geox_physics_compute_ac_risk` — `U_phys` includes data density and boundary-condition scores
- [ ] `cross_audit_transform_lineage` — OpenQuake → map → policy translation logged
- [ ] `geox_physics_judge_verdict` — 888_JUDGE on evacuation trigger scenario
...
- [ ] `geox_physics_compute_ac_risk` — `U_phys` penalized if unvalidated MODFLOW parameter set used
- [ ] `cross_audit_transform_lineage` — DEM → well data → model → forecast chain logged
- [ ] `geox_physics_judge_verdict` — 888_JUDGE on public depletion forecast
...
- [ ] `geox_physics_compute_ac_risk` — `U_phys` penalized if TOUGH2/E300 unvalidated for this basin
- [ ] `cross_audit_transform_lineage` — mesh → EOS → plume → cert chain logged
- [ ] `geox_physics_judge_verdict` — 888_JUDGE on storage permit scenario
...
These checklists are enforced by the `prospect_judge_evaluation`, `geox_physics_judge_verdict`, and `system_compute_metabolize` metabolizers at stage `888` AUDIT.
